    ...
    UINT8 sc_support)
{
    ...
    msg.sig = BTC_SIG_API_CALL;
    msg.pid = BTC_PID_GAP_BLE;
    msg.act = BTC_GAP_BLE_ADD_BOND_DEV_EVT;
    memcpy(arg.add_bond_device.bd_addr, bd_addr, ESP_BD_ADDR_LEN);
    memcpy(arg.add_bond_device.dev_class, dev_class, sizeof(uint8_t) * DEV_CLASS_LEN);
    memcpy(arg.add_bond_device.link_key, link_key, sizeof(uint8_t) * 3);
    arg.add_bond_device.trusted_mask = trusted_mask;
    arg.add_bond_device.is_trusted = is_trusted;
    arg.add_bond_device.key_type = key_type;
    arg.add_bond_device.io_cap = io_cap;
    arg.add_bond_device.pin_length = pin_length;
    arg.add_bond_device.sc_support = sc_support;
