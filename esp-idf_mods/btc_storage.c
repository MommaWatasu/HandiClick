        ...
        BOOLEAN *sc_support)
{
    ...
    int ret = btc_config_get_bin(bdstr, BTC_STORAGE_LINK_KEY_STR, link_key, &size);
    if (ret) {
        ret &= btc_config_get_int(bdstr, BTC_STORAGE_DEV_CLASS_STR, (int *)dev_class);
        ret &= btc_config_get_int(bdstr, BTC_STORAGE_LINK_KEY_TYPE_STR, (int *)key_type);
        ret &= btc_config_get_int(bdstr, BTC_STORAGE_PIN_LENGTH_STR, (int *)pin_length);
        size = sizeof(BOOLEAN);
        ret &= btc_config_get_bin(bdstr, BTC_STORAGE_SC_SUPPORT, (uint8_t *)sc_support, &size);
    }
    btc_config_unlock();

    BTC_TRACE_DEBUG("Storage get rslt %d\n", ret);
    return ret ? BT_STATUS_SUCCESS : BT_STATUS_FAIL;
}
